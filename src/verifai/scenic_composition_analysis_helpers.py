...
class Container:
    ...
    body: Sequence[Any]
    setup_text: str = (
        ""  # raw setup block lines re-indented to 8 spaces; empty for behaviors
    )
...
def extract_setup_text(source_text: str, node: Any) -> str:
    """Extract the body of the setup: block from a ScenarioDef node.

    Uses raw source line slicing (not AST unparse) so that Scenic-specific
    syntax like ``new Object at 0 @ 0`` is preserved verbatim.

    Args:
        source_text: full raw source text of the file containing the node.
        node: a scenic_ast.ScenarioDef with lineno / end_lineno attributes.

    Returns:
        The setup body lines re-indented to 8 spaces and joined by newlines.
        Returns "" if no setup: block is found.
    """
    lines = source_text.splitlines()
    # node.lineno is 1-indexed; slice to the scenario's line range
    scenario_lines = lines[node.lineno - 1 : node.end_lineno]

    setup_body: List[str] = []
    in_setup = False
    setup_indent: Optional[int] = None
    _SIBLING_KEYWORDS = ("compose:", "precondition:", "invariant:", "setup:")
    ...
    for raw_line in scenario_lines:
        ...
        if not in_setup:
            if stripped.startswith("setup:"):
                in_setup = True
                setup_indent = current_indent
                rest = stripped[len("setup:") :].strip()
                if rest:
                    setup_body.append(rest)
            continue

        # Inside setup — stop at a sibling keyword at the same indent level
        if (
            stripped
            and current_indent <= setup_indent
            and any(stripped.startswith(kw) for kw in _SIBLING_KEYWORDS)
        ):
            break

        setup_body.append(raw_line)
    ...
    min_indent = min(
        (len(l) - len(l.lstrip()) for l in setup_body if l.strip()),
        default=0,
    )

    # Re-indent to 8 spaces (depth of setup body inside a wrapper scenario)
    TARGET = 8
    reindented: List[str] = []
    for line in setup_body:
        if not line.strip():
            ...
            reindented.append(" " * (TARGET + extra) + line.lstrip())
        ...
        if isinstance(node, scenic_ast.ScenarioDef):
            containers.append(
                Container(
                    name=node.name,
                    kind="scenario",
                    body=node.compose or (),
                    setup_text=extract_setup_text(text, node),
                )
            )
